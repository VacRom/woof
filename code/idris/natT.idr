...
{- Here is a list of steps in how you evaluate e = evenb (S r)
e
evenb (S r)
evenb (S (S (S (S O))))
true
...
||| given a pair of natural numbers, return its sum <- should be 'product'?
...
--#7: eqp -- given a pair of natural numbers, (a, b), return true if a
  --is equal to b, otherwise return false NOTE I used absolute value
    --rather than a recursive definition. Is this fine? The code works
      --well in a single line.
eqp: pair nat nat -> bool
eqp (mkPair a b) = andp (mkPair (isZero (subp (mkPair a b))) (isZero (subp (mkPair b a))))
...
--#10: ltp -- given a pair of natural numbers, (a, b), return true if a is less than
